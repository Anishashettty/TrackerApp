...
export const togglePermission = async (req: Request, res: Response) => {
	try{
		const { type } = req.body;
		const user = await userService.togglePermission(req.params.id as string, type);
		res.json(user);
	}catch(err: any){
		res.status(400).json({ message: err.message });
	}
...
// Extend Request for multer file (VERY IMPORTANT)
...
export const importUsers = async (req: MulterRequest, res: Response) => {
  try {
    ...
    const header = worksheet.getRow(1).values as any[];
